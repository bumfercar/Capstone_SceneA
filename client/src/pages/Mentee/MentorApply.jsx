    ...
    <>
      ...
      <div style={{maxWidth:480,margin:"80px auto",textAlign:"center",padding:"0 5%"}}>
        <div style={{background:C.white,borderRadius:20,padding:"48px 40px",border:`1px solid ${C.border}`}}>
          ...
          <h2 style={{fontSize:22,fontWeight:700,color:C.text,marginBottom:10}}>신청이 완료됐어요!</h2>
          <p style={{fontSize:14,color:C.textSub,lineHeight:1.7,marginBottom:8}}><strong style={{color:C.navy}}>{mentor.name} 멘토</strong>님의 승인을 기다리고 있어요.</p>
          <p style={{fontSize:13,color:C.textMuted,marginBottom:32}}>{mentor.availableDates[selDateIdx].date} · {selTime} · {sessType==="그룹"?`그룹 ${participants}인`:"1:1 집중 면접"}</p>
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>navigate("/mentor/search")} style={{flex:1,padding:"13px",background:C.white,color:C.text,border:`1.5px solid ${C.border}`,borderRadius:10,fontSize:14,fontWeight:500,cursor:"pointer",fontFamily:"inherit"}}>다른 멘토 보기</button>
            <button onClick={()=>navigate("/dashboard/mentee")} style={{flex:1,padding:"13px",background:C.navy,color:C.white,border:"none",borderRadius:10,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>대시보드로 이동</button>
          </div>
        </div>
      </div>
    </>
  ...
  return(
    <>
      ...
      <main style={{maxWidth:1100,margin:"0 auto",padding:"36px 5% 60px"}}>
        ...
        <button onClick={()=>navigate(-1)} style={{display:"inline-flex",alignItems:"center",gap:6,marginBottom:24,background:"transparent",border:"none",fontSize:13,color:C.textSub,cursor:"pointer",fontFamily:"inherit",padding:0,transition:"color 0.15s"}}
          ...
          <div style={{width:300,flexShrink:0}}>
            <div className="mentor-sticky" style={{background:C.white,borderRadius:16,padding:"24px",border:`1px solid ${C.border}`,position:"sticky",top:88}}>
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
                <div style={{width:52,height:52,borderRadius:"50%",background:mentor.avatarColor,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:700,color:C.white}}>{mentor.name[0]}</div>
                <div>
                  <p style={{fontSize:16,fontWeight:700,color:C.text,marginBottom:2}}>{mentor.name} 멘토</p>
                  <p style={{fontSize:12,color:C.textSub}}>{mentor.company} / {mentor.job} {mentor.years}년차</p>
                </div>
              </div>
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:16}}>
                {mentor.tags.map((t,i)=><span key={i} style={{fontSize:11,padding:"3px 9px",borderRadius:999,background:C.bg,color:C.textSub}}>#{t}</span>)}
              </div>
              <div style={{borderTop:`1px solid ${C.border}`,paddingTop:16}}>
                <p style={{fontSize:12,color:C.text,lineHeight:1.75,fontStyle:"italic",marginBottom:14}}>{mentor.philosophy}</p>
                <p style={{fontSize:11,fontWeight:700,color:C.text,marginBottom:6}}>주요 경력 (Career):</p>
                <ul style={{paddingLeft:14,marginBottom:12}}>
                  {mentor.career.map((c,i)=><li key={i} style={{fontSize:11,color:C.textSub,lineHeight:1.7,marginBottom:2}}>{c}</li>)}
                </ul>
                <p style={{fontSize:11,fontWeight:700,color:C.text,marginBottom:4}}>전문 직무:</p>
                <p style={{fontSize:11,color:C.textSub,marginBottom:12}}>{mentor.techStack}</p>
                <p style={{fontSize:11,fontWeight:700,color:C.text,marginBottom:6}}>면접 집중 코칭 항목:</p>
                <ul style={{paddingLeft:14}}>
                  {mentor.focusItems.map((f,i)=><li key={i} style={{fontSize:11,color:C.textSub,lineHeight:1.7,marginBottom:2}}>{f}</li>)}
                </ul>
              </div>
            </div>
          </div>
          ...
          <div style={{flex:1}}>
            <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,overflow:"hidden"}}>
              ...
              <div style={{padding:"28px 32px",borderBottom:`1px solid ${C.border}`}}>
                <p style={{fontSize:11,fontWeight:700,color:C.textMuted,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:14}}>01 세션 유형 선택</p>
                <div style={{display:"flex",gap:12}}>
                  {[{t:"1:1 집중 면접",d:"60분 · 개인 맞춤형",v:"1:1"},{t:"그룹 면접 연습",d:"60분 · 다대다 실전",v:"그룹"}].map(s=>(
                    <button key={s.v} type="button" onClick={()=>setSessType(s.v)} style={{
                      ...
                      <p style={{fontSize:12,color:sessType===s.v?"rgba(255,255,255,0.65)":C.textMuted}}>{s.d}</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {sessType==="그룹"&&(
                <div style={{padding:"24px 32px",borderBottom:`1px solid ${C.border}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
                    <p style={{fontSize:14,fontWeight:600,color:C.text}}>참여 인원</p>
                    <p style={{fontSize:14,fontWeight:700,color:C.navy}}>{participants}명 (본인 포함)</p>
                  </div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
                    {[2,3,4].map(n=><span key={n} style={{fontSize:11,color:n===participants?C.navy:C.textMuted,fontWeight:n===participants?700:400}}>{n}명{n===4?"(최대)":""}</span>)}
                  </div>
                  <p style={{fontSize:11,color:C.textMuted,marginTop:8}}>* 그룹 세션은 1인당 {Math.round(mentor.point*0.7)}P 적용</p>
                </div>
              )}

              {/* 02 일시 선택 */}
              <div style={{padding:"24px 32px 0",borderBottom:`1px solid ${C.border}`}}>
                <p style={{fontSize:11,fontWeight:700,color:C.textMuted,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:14}}>02 일시 선택 ({mentor.availableDates[selDateIdx].date})</p>
                <div style={{display:"flex",gap:8,marginBottom:0}}>
                  {mentor.availableDates.map((d,i)=>(
                    <button key={i} type="button" onClick={()=>{setSelDateIdx(i);setSelTime("");}} style={{
                      ...
                      fontSize:12,fontWeight:selDateIdx===i?700:400,fontFamily:"inherit",transition:"all 0.15s",
                    }}>{d.date}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{padding:"24px 32px",background:C.bg}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                  <span style={{fontSize:14,color:C.textSub}}>최종 차감 포인트</span>
                  <span style={{fontSize:28,fontWeight:700,color:C.navy,letterSpacing:"-0.03em"}}>{totalPoint} <span style={{fontSize:16}}>P</span></span>
                </div>
                {selTime&&(
                  <div style={{background:C.teal+"14",border:`1px solid ${C.teal}40`,borderRadius:10,padding:"10px 14px",marginBottom:14}}>
                    <p style={{fontSize:13,color:C.teal,fontWeight:600}}>✓ {mentor.availableDates[selDateIdx].date} · {selTime} · {sessType==="그룹"?`그룹 ${participants}인`:"1:1 집중 면접"}</p>
                  </div>
                )}
                ...
                {!canSubmit&&<p style={{fontSize:11,color:C.textMuted,textAlign:"center",marginTop:8}}>날짜와 시간을 선택하면 신청할 수 있어요</p>}
              </div>
            </div>
          </div>
        </div>
